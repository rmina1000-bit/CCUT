...
## 이 문서의 목적
Claude Code가 CCUT 프로젝트를 처음 열 때 반드시 읽어야 할 맥락 문서다.
새 대화를 시작해도 이 문서를 기준으로 판단하고 작업한다.

---

## 제품 한 문장 정의
CCUT은 채팅형 영상 편집 제안 프로그램이다.
사용자는 영상을 올리고, AI가 두 가지 러프컷 안(A/B)을 제안하며,
사용자는 선택하거나 채팅으로 수정 지시하거나 비례바로 조각을 미세 조정한다.
대부분의 사용자는 제안을 선택하고 SNS에 바로 올린다.
