...
def _llm_understand(input_text, recent_messages=None, source_ids=None,
    ...
    prompt = (
        ...
        # [2026-08-08] 예시에서 사람 이름을 뺐다 — 이것이 '정은한' 사건의 씨앗이었다.
        #   기준 없는 말('편집이나 하자')이 오면 모델이 이 예시를 그대로 베껴
        #   instruction 으로 냈고, 그 지어낸 기준으로 17조각 재선별이 3분씩 돌았다.
        #   예시는 사용자 데이터에 실재하는 것으로만 든다.
        "instruction 규칙: edit일 때만 채운다. 반드시 '무엇을 고르는 기준'(예: '바다 나오는 "
    ...
    if kind == "unclear":
        ...
        return _resp("ask_clarification",
                     # [2026-08-08] 예시에서 사람 이름을 뺐다.
                     #   이 문구가 화면에 나가면 대화 기록에 남고, 다음 턴에
                     #   모델이 그것을 사용자의 기준으로 되읽는다(실측: '정은한
                     #   나오는 장면'이 지시문이 되어 17조각 재선별 3분이 돌았다).
                     #   자기가 한 말이 자기 입력이 되는 고리를 끊는다.
                     "어떤 기준으로 고를까요? 예: '바다 나오는 장면만', "
                     "'실내만', '더 길게/짧게'.",
